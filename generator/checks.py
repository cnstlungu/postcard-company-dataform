...
Run from the repository root, with the generator's requirements installed:

    python generator/checks.py

Each check stands for a way the generated data has broken the pipeline before.
...
def declared_calendar():
    """The calendar bounds as declared in the generator and in the Dataform include."""
    py = GENERATE.read_text()
    py_bounds = tuple(
        date(*map(int, m))
        for m in re.findall(r"^CALENDAR_(?:START|END) = date\((\d+), (\d+), (\d+)\)", py, re.M)
    )
    js = (ROOT / "includes" / "calendar.js").read_text()
    js_bounds = tuple(
        date.fromisoformat(m) for m in re.findall(r'^const CALENDAR_(?:START|END) = "([\d-]+)";', js, re.M)
    )
    assert len(py_bounds) == 2, f"could not read CALENDAR_START/END from {GENERATE}"
    assert len(js_bounds) == 2, "could not read CALENDAR_START/END from includes/calendar.js"
    return py_bounds, js_bounds
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    @check("generator and includes/calendar.js declare the same calendar")
    def _():
        py_bounds, js_bounds = declared_calendar()
        assert py_bounds == js_bounds, f"generate.py says {py_bounds}, calendar.js says {js_bounds}"

    # A window reaching outside dim_date used to generate happily and then fail
    # assert_fact_sales_date_key_valid in BigQuery, long after the generator ran.
    ...
    # DATA_END_DATE defaulted to today, so a pinned SEED alone did not pin the
    # dataset: the same settings produced different dates on consecutive days.
    @check("SEED + DATA_END_DATE reproduce a dataset exactly")
    def _():
        first, second = tmp / "repro_1", tmp / "repro_2"
        for out in (first, second):
            assert run_generator(out, SEED=42, DATA_END_DATE="2026-09-14").returncode == 0
    ...
    def _():
        other = tmp / "other_day"
        assert run_generator(other, SEED=42, DATA_END_DATE="2026-09-15").returncode == 0
    ...
    def _():
        (cal_start, cal_end), _js = declared_calendar()
        end = date(2026, 9, 14)
